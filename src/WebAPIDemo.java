...
public class WebAPIDemo {
    ...
    {
        String APIkey = "899fb9fd39055ae65d1559f2030db934";
        ...
        {
            System.out.println(movie.getID() + " | " + movie.getTitle() + " | " + movie.getLanguage() + " | " + movie.getReleaseDate() + " | " + movie.getVoteAverage() + "/10 | " + movie.getPosterPath());
        }
    }
    ...
    public static void parseJSON(String json, ArrayList<Movie> movies)
    {
        ...
        for (int i = 0; i < movieList.length(); i++)
        {
            ...
            String language = movieObj.getString("original_language");
            double voteAverage = movieObj.getDouble("vote_average");
            String releaseDate = movieObj.getString("release_date");

            Movie movie = new Movie(movieTitle, movieID, fullPosterPath, language, voteAverage, releaseDate);
            movies.add(movie);
        }
    }
}